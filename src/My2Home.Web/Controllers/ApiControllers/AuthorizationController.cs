...
using My2Home.Web.ViewModels.Authorization;
    ...
    public class AuthorizationController : BaseApiController<AuthorizationController>
    {
        ...
        public async Task<IActionResult> Exchange(OpenIdConnectRequest request)
        {
            ...
            if (request.IsPasswordGrantType())
            {
                ...
                if(System.DateTime.UtcNow > user.ExpiryDateTime)
                {
                    return BadRequest(new OpenIdConnectResponse
                    {
                        Error = OpenIddictConstants.Errors.AccessDenied,
                        ErrorDescription = "The user validity has been expired."
                    });
                }
        ...
        private async Task<AuthenticationTicket> CreateTicketAsync(
            OpenIdConnectRequest request, ApplicationIdentityUser user,
            AuthenticationProperties properties = null)
        {
            // Create a new ClaimsPrincipal containing the claims that
            // will be used to create an id_token, a token or a code.
            var principal = await _signInManager.CreateUserPrincipalAsync(user);
            var organizationUser = await _organizationUserRepository.GetByIdentityUserIdAsync(user.Id);
            if (principal.Identity is ClaimsIdentity identity)
            {
                // Custom claims
                var customClaims = new List<Claim>
                     {
                          new Claim("My2Home:OrganizationId" , organizationUser?.OrganizationId.ToString()),
                          new Claim("My2Home:CountryId" , user.CountryId.ToString()),
                          new Claim("My2Home:ExpiryDateTime" , user.ExpiryDateTime.Value.ToString("yyyy-MM-dd")),
                     };
            ...
            if (!request.IsAuthorizationCodeGrantType() && !request.IsRefreshTokenGrantType())
            {
                ...
                ticket.SetResources(Startup.Configuration["AppSettings:JWTSettings:Audience"]);
        ...
        private async Task EnsureRoleAsync(string roleName)
        {
            if ((await _roleManager.FindByNameAsync(roleName)) == null)
            {
                ...
                applicationRole.NormalizedName = applicationRole.NormalizedName;
                var result = await _roleManager.CreateAsync(applicationRole);
                if (result == null)
                    throw new System.Exception($"Seeding \"{roleName}\" role failed. Errors: {string.Join(Environment.NewLine, "")}");
            }
        }
        ...
        private async Task<string> CreateRoles()
        {
            try
            {
                ...
                await EnsureRoleAsync("organizationadmin");
